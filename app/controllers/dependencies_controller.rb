class DependenciesController < ApplicationController
  def project
    ...
    if params[:model_name] == "request" 
      ...
      if !params[params[:model_name]][:project_id].empty?
        @users_responsible = Allocation.select('users.id', 'users.nick_name')
                                        .joins(:user)
                                        .joins(user: :company)
                                        .where("company_id = ? and allocations.project_id = ?", 
                                                current_user.company.id, params[params[:model_name]][:project_id])
    ...
    if (params[:model_name] == "request") or (params[:model_name] == "help")
      if !params[params[:model_name]][:project_id].empty?
        @tags = Tag.select(:id, :description).where("project_id = ?", params[params[:model_name]][:project_id])
    ...
    if params[:model_name] == "help_search"
      if !params[:q_sys].empty?
        @users_responsible = Allocation.select('users.id', 'users.nick_name')
                                        .joins(:user)
                                        .joins(user: :company)
                                        .where("company_id = ? and allocations.project_id = ?", 
                                                current_user.company.id, params[:q_sys])
                                        .order("unaccent(users.nick_name)")
      else
        @users_responsible = ''
      end
